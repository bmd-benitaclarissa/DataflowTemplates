...
import com.google.cloud.teleport.kafka.connector.KafkaCheckpointMark;
...
/**
 * The {@link MultipleCDCKafkaTopicsToBigQuery} pipeline is a streaming pipeline which ingests data in JSON format
 ...
 * -Dexec.mainClass=com.google.cloud.teleport.templates.MultipleCDCKafkaTopicsToBigQuery \
 ...
 * JOB_NAME=multiple-cdc-kafka-topics-to-bigquery-$USER-`date +"%Y%m%d-%H%M%S%z"`
 *
...
public class MultipleCDCKafkaTopicsToBigQuery {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(MultipleCDCKafkaTopicsToBigQuery.class);
   ...
   * MultipleCDCKafkaTopicsToBigQuery#run(Options)} method to start the pipeline and invoke {@code
                    ...
                    .apply("SetRecordToProperTableName", ParDo.of(new DoFn<KafkaRecord<String, String>, KV<String, String>>(){
                      ...
                      public void processElement(ProcessContext c){
                        ...
                        c.output(KV.of(element.getCustomizedKey()
                                , element.getKV().getValue().concat("|||insert_offset:").concat(Long.toString(element.getOffset()))));
                      }
                    }))
                              ...
                              public TableSchema getSchema(FailsafeElement<KV<String, String>, String> element) {
                                ...
                                for(idx = 0; idx < jsonFieldsLen; idx++){
                                  ...
                                  fields.add(new TableFieldSchema()
                                                ...
                                                .setType((jsonField.getString("type").equals("int32") || jsonField.getString("type").equals("int64"))
                                                          ? ("INTEGER")
                                                          : ((jsonField.getString("type").equals("double"))
                                                              ? ("FLOAT")
                                                              : ((jsonField.getString("type").equals("boolean"))
                                                                ? ("BOOLEAN")
                                                                : ("STRING")
                                                                )
                                                              )
                                ...
                                fields.add(new TableFieldSchema()
                                                .setName("insert_timestamp")
                                                .setType("INTEGER")
                                                .setMode("NULLABLE")
                                );
                                fields.add(new TableFieldSchema()
                                                .setName("insert_offset")
                                                .setType("INTEGER")
                                                .setMode("NULLABLE")
                                );
  ...
  public abstract static class WriteKafkaMessageErrors
    ...
    public static Builder newBuilder() {
      return new AutoValue_MultipleCDCKafkaTopicsToBigQuery_WriteKafkaMessageErrors.Builder();
    }
